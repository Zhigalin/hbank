		<div id="pages">
			<ul class="pagination">
				...
				<?php } ?>
				<?php for ($i = $pagination['start']; $i <= $pagination['end']+1; $i++) { ?>
					...
					<?php } ?>
				<?php } ?>
				<?php if ($pagination['page'] != intval($pagination['end'])+1) { ?>
					<li><a href="<?=$pagination['url_page'].($pagination['page'] + 1)?>">&gt;</a></li>
					<li><a href="<?=$pagination['url_page'].(intval($pagination['end'])+1)?>">&raquo;</a></li>
				<?php } ?>
			</ul>
		</div>